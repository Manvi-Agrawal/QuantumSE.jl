...
function get_stabilizer_and_nbr(R::Integer, C::Integer)
    num_qubits = R*C
...
#    println("Z 2q: $(z_2q)")
    ...
    for j in eachindex(z_2q)
        for z_adj in _adj2_ver( C, z_2q[j])
            stabilizer[r+j, num_qubits+z_adj] = true
...
function check_surface_code_decoder(R::Integer, C::Integer)
    ...
    t0 = time()
    begin
        (stabilizer, X_nbr, Z_nbr) = get_stabilizer_and_nbr(R, C)
    ...
    t1 = time()
    begin
        (ρ01, ϕ_l, cfg1) = get_config(stabilizer, phases, X_nbr, Z_nbr, R, C)
    ...
    begin
        ...
        for cfg in cfgs1
            if !generate_constraints(
                cfg.ρ, ρ01, (ϕ_l #=& ϕ_z2=#, cfg.ϕ[1], cfg.ϕ[2]),
                )
                res = false
                break
            end
...
check_surface_code_decoder(5, 3) # precompile time
@info "PRECOMPILE done..."
println("-------------------------------------------")

open("rect_buggy_sc.csv", "w") do io
