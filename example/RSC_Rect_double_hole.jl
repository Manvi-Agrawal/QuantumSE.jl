# Indexing Error: Take more close look on bounds
...
function get_dims(matrix)
    return (length(matrix[:, 1]), length(matrix[1, :]))
end
...
function get_config(stabilizer, phases, X_nbr, Z_nbr, R::Integer, C::Integer)
    ...
    println("Config stabilizer dims: $(get_dims(stabilizer))")


    ρ01 = from_stabilizer(num_qubits-2, stabilizer, phases, ctx)
    ρ1 = copy(ρ01)
...
function get_phases(R::Integer, C::Integer, nh::Integer)
    num_qubits = R*C - nh
...
function get_holes(stabilizer, nq::Integer, X_nbr, Z_nbr, holes)
    ...
    a_new = delete_matrix_cols(a_temp, del_col) 
    ...
    println("Col deletion:: holes = $(holes), Ranks-AB: $(rA), $(rB)")


    k_temp = stabilizer[1:nq-1, :]
    k_new = delete_matrix_cols(k_temp, del_col) 
    println("k_new dims: $(get_dims(k_new))")
    ...
    println("LX + Col deletion:: , holes = $(holes), Ranks-KL: $(rK), $(rL)")
    
    
    
    if rQ-rL==2
        println("YAY: holes = $(holes), Ranks-(Orig,Lx_C): $(rQ), $(rL)")
    end
    
    M = [Int64(x.val)==1 for x in L]
    return remove_false_rows(M)
...
function check_surface_code_decoder(R::Integer, C::Integer)
    ...
    good_holes = [ [43 42], [44, 26], [44 35], [45 36], [45 44] ]
    ...
    for holes in good_holes
        println("Iterating for holes=$(holes)")
        ...
        println("$(d), $(holes), $(res_d), $(R*C), $(t4-t0), $(t1-t0), $(t2-t1), $(t3-t2), $(t4-t3)")
