...
@qprog surface_code_decoder (d) begin
    ...
    s_x = [surface_code_x_m(j) for j in 1:6]
    s_z = [surface_code_z_m(j) for j in 1:6]
...
function check_surface_code_decoder(d::Integer)
    ...
    begin
        ...
        for r in 1:6
            for z_adj in Z_idxs[r]
                stabilizer[6+r, num_qubits+z_adj] = true
            end
        end
        ...
        stabilizer[num_qubits, 1] = true
        stabilizer[num_qubits, 6] = true
        stabilizer[num_qubits, 11] = true
        stabilizer[num_qubits, 16] = true
        stabilizer[num_qubits, 21] = true
...
println("$(d): $(res) $(d*d) $(all) $(init) $(qse) $(smt)")
